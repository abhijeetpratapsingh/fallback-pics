...
export async function generateImage(
  ...
  switch (preset) {
    ...
    case "chart":
      ...
      break;
    case "thumbnail":
      svg = generateThumbnailSVG(scaledWidth, scaledHeight, {
        text,
        label: params.thumbnailLabel,
        style: params.thumbnailStyle,
        theme: params.thumbnailTheme,
        bg: params.thumbnailBg,
        accent: params.thumbnailAccent,
        color: params.thumbnailColor,
        seed: params.thumbnailSeed,
      });
      break;
    default:
      svg = generateStandardSVG(
        scaledWidth,
        scaledHeight,
        bgColor,
        textColor,
...
function generateStandardSVG(
  ...
  const fontSize =
    customFontSize || Math.max(14, Math.min(width, height) * 0.1);
