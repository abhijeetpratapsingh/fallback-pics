export interface ImageParams {
  ...
  textColor: string;
  text?: string;
  preset?: 'square' | 'avatar' | 'banner' | 'random' | 'blur' | 'skeleton' | 'ai' | 'animated' | 'chart';
  animationType?: 'skeleton' | 'pulse' | 'wave' | 'shimmer' | 'gradient' | 'dots';
  chartType?: 'bar' | 'pie' | 'line' | 'area' | 'donut' | 'scatter' | 'radar' | 'heatmap';
  font?: string;
  fontSize?: number;
  retina?: number;
  context?: string;
...
export class Router {
  ...
  parse(): ImageParams {
    ...
    if (host.includes('img.fallback.pics') || host.includes('https://fallback.pics/api/v1')) {
    ...
    // Handle presets
    if (['square', 'avatar', 'banner', 'random', 'blur', 'skeleton', 'ai', 'animated', 'chart'].includes(first)) {
    ...
    let bgColor = (preset === 'animated' || preset === 'skeleton' || preset === 'chart') ? '#FFFFFF' : '#7C3AED';
    let textColor = (preset === 'animated' || preset === 'skeleton' || preset === 'chart') ? '#6B7280' : '#FFFFFF';
    ...
    const bgParam = this.url.searchParams.get('bg') || this.url.searchParams.get('bgColor');
    const textParam = this.url.searchParams.get('fg') || this.url.searchParams.get('textColor');
    
    if (bgParam) bgColor = this.normalizeColor(bgParam);
    if (textParam) textColor = this.normalizeColor(textParam);

    // Custom text
    const text = this.url.searchParams.get('text') || 
    ...
    if (preset === 'animated' && segments[1]) {
      const validTypes = ['skeleton', 'pulse', 'wave', 'shimmer', 'gradient', 'dots'];
    ...
    // Accessibility
    const reducedMotion = this.url.searchParams.get('reducedMotion') === 'true' ||
                         this.url.searchParams.get('reduced-motion') === 'true';
    
    // Chart type
    let chartType: ImageParams['chartType'] | undefined;
    if (preset === 'chart' && segments[1]) {
      const validTypes = ['bar', 'pie', 'line', 'area', 'donut', 'scatter', 'radar', 'heatmap'];
      if (validTypes.includes(segments[1])) {
  ...
  private validateFormat(format: string): ImageParams['format'] {
    ...
    if (validFormats.includes(normalized)) {
      ...
      return (normalized === 'jpeg' ? 'jpg' : normalized) as ImageParams['format'];
    }
    return 'svg';
  }
